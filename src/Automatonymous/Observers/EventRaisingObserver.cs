...
namespace Automatonymous.Observers
{
        ...
        where TInstance : class
    {
        ...
        public EventRaisingObserver(IDictionary<string, StateMachineEvent<TInstance>> eventCache)
        {
        ...
        public void OnNext(EventRaising<TInstance> value)
        {
            ...
            if (_eventCache.TryGetValue(value.Event.Name, out stateMachineEvent))
                stateMachineEvent.EventRaising.OnNext(value);
        }

        public void OnError(Exception error)
        {
            foreach (var stateMachineEvent in _eventCache.Values)
                stateMachineEvent.EventRaising.OnError(error);
        }

        public void OnCompleted()
        {
            foreach (var stateMachineEvent in _eventCache.Values)
                stateMachineEvent.EventRaising.OnCompleted();
        }
    }
}